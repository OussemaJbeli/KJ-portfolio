    ...
    <div class="comment">
        ...
        <div class="user_comment">
            <p class="message">
                ...
                <div class="contents">{{ comment.text }}</div>
            </p>
            <div class="buttons">
                <div class="actions">
                    ...
                    <button @click="showReplyForm = !showReplyForm">{{ showReplyForm ? 'Cancel' : 'Reply' }}</button>
                </div>
                <div class="replayFrame" v-if="showReplyForm">
                    <div class="user_logo">
                        <i class="fa-solid fa-user"></i>
                    </div>
                    <div class="messageBoxReplay">
                        <textarea v-model="newReply" required="" placeholder="write a comment..." type="text" id="messageInput1"  cols="30" rows="10"></textarea>
                        <div class="footer">
                            <button id="sendButton" @click="addReply">
                ...
                <button v-if="comment.replies.length" @click="toggleReplies" class="replay">
                    <span>{{ comment.repliesNum }} Replies</span>
                </button>
    ...
    methods: {
      addReply() {
        if (this.newReply.trim()) {
          ...
          this.$emit('reply-added', this.comment);
        }
      },
      toggleReplies() {
      ...
      onReplyAdded(updatedComment) {
        this.$emit('reply-added', updatedComment);
      }
    }
  };
  </script>
